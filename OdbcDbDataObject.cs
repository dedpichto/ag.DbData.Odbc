...
namespace ag.DbData.Odbc
{
    ...
    public class OdbcDbDataObject : DbDataObject
    {
        ...
        public OdbcDbDataObject(ILogger<IDbDataObject> logger, IOptions<DbDataSettings> options) : base(logger, options) { }
        #endregion
        ...
        private async Task<int> innerExecuteAsync(string query, CancellationToken cancellationToken, int timeout = -1)
        {
            try
            {
                ...
                {
                    int rows;
                    using (var asyncConnection = new OdbcConnection(Connection.ConnectionString))
                    {
