![Nuget](https://img.shields.io/nuget/v/ag.DbData.Odbc)
...
4. Inject IOdbcDbDataFactory into your classes:
...
- [ag.DbData.Odbc](https://www.nuget.org/packages/ag.DbData.Odbc/)
...
```csharp
IServiceCollection AddAgOdbc(this IServiceCollection services)
```

Appends the registration of ```IDbDataFactory``` and ```IDbDataObject``` to ```IServiceCollection```.

```csharp
IServiceCollection AddAgOdbc(this IServiceCollection services, IConfigurationSection configurationSection)
```
...
```csharp
IServiceCollection AddAgOdbc(this IServiceCollection services, Action<DbDataSettings> configureOptions)
```
